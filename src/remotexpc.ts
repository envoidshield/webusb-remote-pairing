...
export var xpcDebugLog: ((msg: string) => void) | null = null
export function setXpcDebugLog(fn: ((msg: string) => void) | null) { xpcDebugLog = fn }
...
export class RemoteXpcConnection {
    ...
    private inbox: Record<string, XpcValue>[] = []
    private waiters: Array<(obj: Record<string, XpcValue>) => void> = []
    private msgQueue: Array<{ parsed: Record<string, XpcValue> | null, flags: number }> = []
    private msgWaiters: Array<(msg: { parsed: Record<string, XpcValue> | null, flags: number }) => void> = []
    ...
    _log(msg: string) {
        if (this.log) this.log('[RemoteXPC] ' + msg)
    }
    ...
    feed(data: Uint8Array) {
        ...
        while (true) {
            ...
            let parsed: Record<string, XpcValue> | null = null
            if (msg.body && msg.body.length >= 16) {
                parsed = decodeXpcBody(msg.body)
                if (parsed) this._log('  Parsed dict keys: ' + Object.keys(parsed).join(', '))
            }
            ...
            if (parsed && parsed.value !== undefined && parsed.value !== null) {
                if (this.waiters.length > 0) this.waiters.shift()!(parsed)
                else this.inbox.push(parsed)
            }
            if (this.onMessage) this.onMessage(parsed, msg.flags)
        }
    }

    sendDict(dict: Record<string, XpcValue>) {
    ...
    waitForAnyMessage(timeoutMs: number = 120000): Promise<{ parsed: Record<string, XpcValue> | null, flags: number }> {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                if (idx >= 0) this.msgWaiters.splice(idx, 1)
                reject(new Error('RemoteXPC waitForAnyMessage timed out after ' + timeoutMs + 'ms'))
            }, timeoutMs)
            const onMsg = (msg: { parsed: Record<string, XpcValue> | null, flags: number }) => {
                clearTimeout(timer)
                resolve(msg)
            }
    ...
    waitForDict(timeoutMs: number = 120000): Promise<Record<string, XpcValue>> {
        ...
        return new Promise((resolve, reject) => {
            ...
            const onObj = (obj: Record<string, XpcValue>) => {
                clearTimeout(timer)
                resolve(obj)
            }

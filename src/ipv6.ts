...
export var ipv6DebugLog: ((msg: string) => void) | null = null
export function setIpv6DebugLog(fn: ((msg: string) => void) | null) { ipv6DebugLog = fn }

function ipv6Debug(msg: string) {
    if (ipv6DebugLog) ipv6DebugLog(msg)
}

function ipv6HexDump(data: Uint8Array, maxBytes: number = 32): string {
...
export interface IPv6Packet {
    ...
    nextHeader: number
    hopLimit: number
    srcAddr: Uint8Array  // 16 bytes
    dstAddr: Uint8Array  // 16 bytes
    payload: Uint8Array
}

export function parseIPv6(data: Uint8Array): IPv6Packet | null {
    ...
    const protoNames: { [k: number]: string } = { 6: 'TCP', 17: 'UDP', 58: 'ICMPv6', 0: 'HOPBYHOP' }
...
export function buildIPv6(nextHeader: number, hopLimit: number, srcAddr: Uint8Array, dstAddr: Uint8Array, payload: Uint8Array): Uint8Array {
 ...
 */
export function skipExtensionHeaders(nextHeader: number, payload: Uint8Array): { nextHeader: number, payload: Uint8Array } {
    ...
    while (extensionHeaders.has(nextHeader) && payload.length >= 8) {
        ...
        if (payload.length < extLength) break
...
export function ipv6ToString(addr: Uint8Array): string {
    const groups: string[] = []
...
export function ipv6FromString(s: string): Uint8Array {
...
export function ipv6IsLinkLocal(addr: Uint8Array): boolean {
 ...
 */
export function ipv6LinkLocalFromMac(mac: Uint8Array): Uint8Array {
    const addr = new Uint8Array(16)
    addr[0] = 0xFE
    addr[1] = 0x80
    // bytes 2-7 = 0
 ...
 */
export function solicitedNodeAddress(addr: Uint8Array): Uint8Array {
...
export function icmpv6Checksum(srcAddr: Uint8Array, dstAddr: Uint8Array, icmpPayload: Uint8Array): number {
 ...
 */
export function buildNeighborSolicitation(targetAddr: Uint8Array, sourceMac: Uint8Array, srcIpv6: Uint8Array): Uint8Array {
...
export function parseNeighborAdvertisement(icmpPayload: Uint8Array): NeighborAdvertisement | null {
    if (icmpPayload.length < 24) return null
    if (icmpPayload[0] !== ICMPV6_NEIGHBOR_ADVERTISEMENT) return null
    ...
    let targetMac: Uint8Array | null = null
    let offset = 24

...
export var http2DebugLog: ((msg: string) => void) | null = null
export function setHttp2DebugLog(fn: ((msg: string) => void) | null) { http2DebugLog = fn }

function http2Debug(msg: string) {
    if (http2DebugLog) http2DebugLog(msg)
}

function http2HexDump(data: Uint8Array, maxBytes: number = 48): string {
...
export function getClientPreface(): Uint8Array {
...
export function parseHttp2Frame(data: Uint8Array): [Http2Frame, Uint8Array] | null {
    if (data.length < 9) {
    ...
    const typeNames: { [k: number]: string } = {
...
export function buildHttp2Frame(type: number, flags: number, streamId: number, payload: Uint8Array): Uint8Array {
...
export function buildSettingsFrame(settings: Http2Settings): Uint8Array {
    var entries: Array<[number, number]> = []
...
export function buildSettingsAck(): Uint8Array {
    return buildHttp2Frame(FRAME_SETTINGS, FLAG_ACK, 0, new Uint8Array(0))
}

export function buildWindowUpdate(streamId: number, increment: number): Uint8Array {
...
export function buildDataFrame(streamId: number, data: Uint8Array, endStream?: boolean): Uint8Array {
...
export function buildHeadersFrame(streamId: number): Uint8Array {
    return buildHttp2Frame(FRAME_HEADERS, FLAG_END_HEADERS, streamId, new Uint8Array(0))
}

export function parseSettings(payload: Uint8Array): Http2Settings {
    ...
    for (var i = 0; i + 6 <= payload.length; i += 6) {
        ...
        switch (id) {
            case SETTINGS_HEADER_TABLE_SIZE: settings.headerTableSize = value; break
            case SETTINGS_ENABLE_PUSH: settings.enablePush = value === 1; break
            case SETTINGS_MAX_CONCURRENT_STREAMS: settings.maxConcurrentStreams = value; break
            case SETTINGS_INITIAL_WINDOW_SIZE: settings.initialWindowSize = value; break
            case SETTINGS_MAX_FRAME_SIZE: settings.maxFrameSize = value; break
            case SETTINGS_MAX_HEADER_LIST_SIZE: settings.maxHeaderListSize = value; break
        }
    }
    return settings
}
...
export class Http2Connection {
    ...
    openedStreams: { [streamId: number]: boolean }
    onReady: (() => void) | null
    ...
    _log(msg: string) {
        if (this.log) this.log('[HTTP/2] ' + msg)
    }
    ...
    _handleFrame(frame: Http2Frame) {
        var typeNames: { [k: number]: string } = {
            0: 'DATA', 1: 'HEADERS', 2: 'PRIORITY', 3: 'RST_STREAM',
            4: 'SETTINGS', 5: 'PUSH_PROMISE', 6: 'PING', 7: 'GOAWAY', 8: 'WINDOW_UPDATE'
        }
    ...
    sendData(streamId: number, data: Uint8Array) {
        if (!this.openedStreams[streamId]) {

...
function calcPadding(l: number): number {
...
export function decodeXpcObject(data: Uint8Array, offset = 0): { value: XpcValue; next: number } {
    ...
    switch (t) {
        ...
        case XPC_TYPE_DICT: {
            ...
            const dict: Record<string, XpcValue> = {}
...
export function decodeXpcBody(data: Uint8Array): Record<string, XpcValue> | null {
    if (data.length < 16) return null
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength)
    if (view.getUint32(0, true) !== XPC_OBJECT_MAGIC) return null
    if (view.getUint32(4, true) !== BODY_VERSION) return null
    ...
    if (result.value && typeof result.value === 'object' && !Array.isArray(result.value) && !(result.value instanceof Uint8Array)) {
        return result.value as Record<string, XpcValue>
    }
    return null
}

function encodeDictionaryKey(buf: number[], key: string): void {
...
function encodeObjectPayload(value: XpcValue): number[] {
    ...
    if (typeof value === 'string') {
        ...
        for (let i = 0; i < pad; i++) payload.push(0)
        return payload
    }
    if (value instanceof Uint8Array) {
        writeU32(payload, XPC_TYPE_DATA)
        writeU32(payload, value.length)
        for (let i = 0; i < value.length; i++) payload.push(value[i])
    ...
    const dict = value as Record<string, XpcValue>
    const inner: number[] = []
...
export function encodeXpcBody(dict: Record<string, XpcValue>): Uint8Array {
...
export function buildXpcEmptyDict(): Uint8Array {
    return encodeXpcBody({})
}

export function getChildMap(m: Record<string, XpcValue>, ...keys: string[]): Record<string, XpcValue> {
    let cur: XpcValue = m
    for (const k of keys) {
        if (!cur || typeof cur !== 'object' || Array.isArray(cur) || cur instanceof Uint8Array) {
            throw new Error(`getChildMap: missing '${k}'`)
        }
        cur = (cur as Record<string, XpcValue>)[k]
    }
    if (!cur || typeof cur !== 'object' || Array.isArray(cur) || cur instanceof Uint8Array) {
        throw new Error('getChildMap: not a map')
    }
    return cur as Record<string, XpcValue>
}

export function getChildString(m: Record<string, XpcValue>, ...keys: string[]): string {
    let cur: XpcValue = m
    for (const k of keys) {
        if (!cur || typeof cur !== 'object' || Array.isArray(cur) || cur instanceof Uint8Array) {
...
export function buildRemoteXpcMessage(flags: number, messageId: number, body: Uint8Array | null): Uint8Array {
    ...
    if (body) buf.set(body, 24)
    return buf
}

export function encodeRemoteXpcDict(messageId: number, dict: Record<string, XpcValue>, extraFlags = 0): Uint8Array {
...
export function parseRemoteXpcMessage(data: Uint8Array): [RemoteXpcMessage, Uint8Array] | null {
    if (data.length < 24) return null
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength)
    if (view.getUint32(0, true) !== REMOTEXPC_MAGIC) return null

...
export var tcpDebugLog: ((msg: string) => void) | null = null
export function setTcpDebugLog(fn: ((msg: string) => void) | null) { tcpDebugLog = fn }

function tcpDebug(msg: string) {
    if (tcpDebugLog) tcpDebugLog(msg)
}

function tcpHexDump(data: Uint8Array, maxBytes: number = 32): string {
...
export function parseTcp(data: Uint8Array): TcpSegment | null {
    ...
    if (flags & TH_FIN) flagNames.push('FIN')
    if (flags & TH_SYN) flagNames.push('SYN')
    if (flags & TH_RST) flagNames.push('RST')
    if (flags & TH_PSH) flagNames.push('PSH')
    if (flags & TH_ACK) flagNames.push('ACK')
...
export function buildTcp(srcPort: number, dstPort: number, seqNum: number, ackNum: number, flags: number, windowSize: number, payload: Uint8Array, options?: Uint8Array): Uint8Array {
 ...
 */
export function tcpChecksum(srcAddr: Uint8Array, dstAddr: Uint8Array, tcpSegment: Uint8Array): number {
    ...
    buf[36] = 0; buf[37] = 0; buf[38] = 0; buf[39] = 6 // next header = TCP
 ...
 */
export function buildTcpWithChecksum(
    srcAddr: Uint8Array, dstAddr: Uint8Array,
    srcPort: number, dstPort: number,
    seqNum: number, ackNum: number,
    flags: number, windowSize: number,
    payload: Uint8Array,
    options?: Uint8Array
): Uint8Array {
...
export class TcpConnection {
    ...
    _log(msg: string) {
        if (this.log) this.log(`[TCP ${this.srcPort}->${this.dstPort}] ${msg}`)
    }
    ...
    connect() {
        ...
        const syn = buildTcpWithChecksum(
    ...
    handleSegment(seg: TcpSegment) {
        var flagStr = ''
        if (seg.flags & TH_SYN) flagStr += 'SYN '
        if (seg.flags & TH_ACK) flagStr += 'ACK '
        if (seg.flags & TH_FIN) flagStr += 'FIN '
        if (seg.flags & TH_RST) flagStr += 'RST '
        if (seg.flags & TH_PSH) flagStr += 'PSH '
        ...
        const computedCksum = tcpChecksum(this.dstAddr, this.srcAddr, 
            new Uint8Array(seg.dataOffset * 4 + seg.payload.length))
        ...
        switch (this.state) {
            ...
            default:
                if (seg.flags & TH_RST) {
                    ...
                    if (this.onError) this.onError('Connection reset by peer')
                }
        }
    }

    _handleSynSent(seg: TcpSegment) {
        ...
        if (seg.flags & TH_RST) {
            ...
            if (this.onError) this.onError('Connection refused')
            return
        }
        ...
        if ((seg.flags & (TH_SYN | TH_ACK)) === (TH_SYN | TH_ACK)) {
            ...
            const ack = buildTcpWithChecksum(
                this.srcAddr, this.dstAddr,
                this.srcPort, this.dstPort,
                this.localSeq, this.localAck,
                TH_ACK, 65535,
                new Uint8Array(0)
            )
            this.sendPacket(ack)
    ...
    _handleEstablished(seg: TcpSegment) {
        if (seg.flags & TH_RST) {
            this.state = TcpState.CLOSED
            this._log('RST received')
            if (this.onError) this.onError('Connection reset')
            return
        }
        ...
        if (seg.payload.length > 0) {
            ...
            if (this.onData) this.onData(seg.payload)
        }
    ...
    _handleFinWait1(seg: TcpSegment) {
        if (seg.flags & TH_ACK) {
            if (seg.flags & TH_FIN) {
                // Simultaneous close
                this.localAck = (seg.seqNum + 1) >>> 0
    ...
    _handleFinWait2(seg: TcpSegment) {
        if (seg.flags & TH_FIN) {
            this.localAck = (seg.seqNum + 1) >>> 0
            const ack = buildTcpWithChecksum(
                this.srcAddr, this.dstAddr,
    ...
    send(data: Uint8Array) {
        ...
        const seg = buildTcpWithChecksum(
            this.srcAddr, this.dstAddr,
            this.srcPort, this.dstPort,
            this.localSeq, this.localAck,
            TH_ACK | TH_PSH, 65535,
    ...
    close() {
        if (this.state === TcpState.ESTABLISHED) {
            ...
            const fin = buildTcpWithChecksum(
                this.srcAddr, this.dstAddr,
                this.srcPort, this.dstPort,
                this.localSeq, this.localAck,
                TH_FIN | TH_ACK, 65535,

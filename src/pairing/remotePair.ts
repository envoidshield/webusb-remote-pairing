...
import {
    PAIR_STATE_EXCHANGE_REQUEST,
    PAIR_STATE_VERIFY_REQUEST,
    TLV_ENCRYPTED_DATA,
    TLV_IDENTIFIER,
    TLV_INFO,
    TLV_METHOD,
...
function nonceWithTag(tag: string): Uint8Array {
...
function hkdfSha512(ikm: Uint8Array, salt: Uint8Array | undefined, info: string, length: number): Uint8Array {
...
export async function setupNewPairingGetHostKey(
    connOrChannel: ControlChannel | { sendDict: Function, waitForDict: Function },
    selfId: SelfIdentity,
    onStatus?: (msg: string) => void,
): Promise<string> {
    const ch = connOrChannel instanceof ControlChannel
        ? connOrChannel
        : new ControlChannel(connOrChannel as any)
...
async function exchangeDeviceInfo(ch: ControlChannel, sessionKey: Uint8Array, selfId: SelfIdentity) {
...
function setupCiphers(sessionKey: Uint8Array): SessionCiphers {
    return {
...
function updateNonce(c: SessionCiphers) {
    const view = new DataView(c.nonce.buffer)
    view.setUint32(0, c.sequence >>> 0, true)
    view.setUint32(4, 0, true)
}

async function createUnlockKey(ch: ControlChannel, c: SessionCiphers): Promise<string> {
    const req = {
        request: {
            _0: {
    ...
    if (!cip) throw new Error('createUnlockKey: missing encrypted payload')
    ...
    if (typeof hostKey !== 'string') throw new Error('createUnlockKey: missing hostKey')
    return hostKey
}

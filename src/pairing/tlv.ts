...
export class TlvBuffer {
    ...
    writeData(type: number, data: Uint8Array) {
        let offset = 0
        while (offset < data.length) {
            ...
            for (let i = 0; i < chunk; i++) this.chunks.push(data[offset + i])
            offset += chunk
        }
        if (data.length === 0) {
            this.chunks.push(type & 0xff)
    ...
    writeByte(type: number, v: number) {
...
export function tlvReadCoalesced(data: Uint8Array, type: number): Uint8Array {
    const out: number[] = []
    let i = 0
    while (i + 2 <= data.length) {
        ...
        if (t === type) {
            for (let j = 0; j < l; j++) out.push(data[i + j])
        }
        i += l
    }

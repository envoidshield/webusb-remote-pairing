...
const N_HEX = 'FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD129024E088A67CC74' +
...
export function padToN(n) {
    const b = bigToBytes(n);
    if (b.length >= SRP_N_BYTES)
        return b;
...
export function srpXHash(salt, password = SRP_PASSWORD) {
...
export function newSrpClient(salt, serverPublicB, password = SRP_PASSWORD) {
    ...
    return {
        ...
        sessionKey,
        verifyServerProof: (p) => {
            if (p.length !== serverExpected.length)
                return false;
            let diff = 0;
            for (let i = 0; i < p.length; i++)
                diff |= p[i] ^ serverExpected[i];
            return diff === 0;
        },
    };
}

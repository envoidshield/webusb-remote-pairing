...
export function encodeOpack(m) {
    const keys = Object.keys(m);
    if (keys.length > 0xF)
        throw new Error('opack dict exceeds max size 0xF');
    const parts = [0xE0 | keys.length];
...
function writeData(parts, b) {
    writeLengthId(parts, 0x70, b.length);
    for (let i = 0; i < b.length; i++)
        parts.push(b[i]);
}
function writeLengthId(parts, t, l) {
    const id = createIdentifierWithLength(t, l);
    for (let i = 0; i < id.length; i++)
        parts.push(id[i]);
}
function createIdentifierWithLength(t, l) {
    if (l <= 0xF)
        return [t | l];
    if (l < 0x20) {
        const inc = t + (1 << 4);
...
export function buildHostDeviceInfo(accountId) {
    return encodeOpack({
        accountID: accountId,
        altIRK: HOST_ALT_IRK,

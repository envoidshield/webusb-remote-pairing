        ...
        case 'dict': {
            ...
            for (let i = 0; i + 1 < children.length; i += 2) {
                if (children[i].tagName !== 'key')
                    throw new Error('Invalid plist dictionary key');
...
export function decodePlist(data) {
    const document = new DOMParser().parseFromString(textDecoder.decode(data), 'application/xml');
    const parserError = document.querySelector('parsererror');
    if (parserError)
        throw new Error(`Invalid plist XML: ${parserError.textContent || 'parse error'}`);
    const root = document.documentElement.firstElementChild;
    if (!root)
        throw new Error('Invalid plist XML: missing root value');
...
export function buildPlistFrame(value) {

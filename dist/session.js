...
export class RemotePairingSession {
    ...
    async readTrustedLockdownDeviceInfo() {
        if (!this.remotePairingSvc || !this.lockdownPort) {
            throw new Error('Trusted lockdown service is unavailable');
        }
        let lockdown = null;
        const connection = await this.connectServiceTcp(this.remotePairingSvc.address, this.lockdownPort, 52000, data => { if (lockdown)
            lockdown.feed(data); });
        try {
    ...
    async readDeviceInfoBestEffort() {
        ...
        for (let attempt = 1; attempt <= 4; attempt++) {
            ...
            try {
                ...
                this.emitLog(`Post-trust lockdown reconnect ${attempt}/4 failed: ${lastError.message}`);
            }
        }
